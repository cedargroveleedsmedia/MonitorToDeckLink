...
namespace MonitorToDeckLink
{
    ...
    public interface IDeckLinkOutput2
    {
        ...
        [PreserveSig] int SetAncillaryData([MarshalAs(UnmanagedType.Interface)] object ancillary);
        ...
        [PreserveSig] int StopScheduledPlayback(long stopPlaybackAtTime, out long actualStopTime, long timeScale);
        [PreserveSig] int IsScheduledPlaybackRunning(out bool active);
        [PreserveSig] int GetScheduledStreamTime(long desiredTimeScale, out long streamTime, out double playbackSpeed);
        [PreserveSig] int GetReferenceStatus(out int referenceStatus);
        [PreserveSig] int EnableAudioOutput(int sampleRate, int sampleType, uint channelCount, int streamType);
        [PreserveSig] int DisableAudioOutput();
        [PreserveSig] int WriteAudioSamplesSync(IntPtr buffer, uint sampleFrameCount, out uint sampleFramesWritten);
        [PreserveSig] int BeginAudioPreroll();
        [PreserveSig] int EndAudioPreroll();
        [PreserveSig] int ScheduleAudioSamples(IntPtr buffer, uint sampleFrameCount, long streamTime, long timeScale, out uint sampleFramesWritten);
        ...
        [PreserveSig] int SetAudioCallback([MarshalAs(UnmanagedType.Interface)] object callback);
        [PreserveSig] int GetOutputVideoFrameState([MarshalAs(UnmanagedType.Interface)] IDeckLinkMutableVideoFrame2 frame, out int state);
    }
    ...
    public interface IDeckLinkMutableVideoFrame2
    {
        ...
        [PreserveSig] int GetTimecode(int format, [MarshalAs(UnmanagedType.Interface)] out object timecode);
        [PreserveSig] int GetAncillaryData([MarshalAs(UnmanagedType.Interface)] out object ancillary);
        [PreserveSig] int SetFlags(int newFlags);
        [PreserveSig] int SetTimecode(int format, [MarshalAs(UnmanagedType.Interface)] object timecode);
        [PreserveSig] int SetTimecodeFromComponents(int format, byte hours, byte minutes, byte seconds, byte frames, int flags);
        [PreserveSig] int SetAncillaryData([MarshalAs(UnmanagedType.Interface)] object ancillary);
        [PreserveSig] int SetTimecodeUserBits(int format, uint userBits);
        [PreserveSig] int SetInterfaceProvider([MarshalAs(UnmanagedType.Interface)] object provider);
    }
